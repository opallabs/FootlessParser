...
public func noneOf(_ strings: [String]) -> Parser<Character, Character> {
    ...
    return Parser { input in
        ...
        for (string, characters) in strings {
            ...
            guard input.count >= characters.count else { continue }
            let endIndex = input.index(input.startIndex, offsetBy: characters.count)
...
func position(of index: String.Index, in string: String) -> (line: Range<String.Index>, row: Int, pos: Int) {
